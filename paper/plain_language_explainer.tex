...
percent of bank alerts down as false alarms), while quietly missing the real
networks.
...
a system can be ``98 percent accurate'' by simply saying \emph{nothing is
...
\item A tight knot of \textbf{pass-through accounts} that hold money for almost
      no time looks like a tight knot of firms whose \textbf{bid prices cluster}
...
quote numbers that actually exist in the data, and a filter it cannot bypass
blocks it from saying anyone is guilty. Every answer carries a reminder that
the system screens cases and does not judge them.
\end{enumerate}
...
IBM AMLworld & Banking & Five million synthetic (computer-generated) bank transactions with every laundering scheme labelled. Tried as a large-scale test, where the ordinary methods scored below chance; the paper explains why and uses it instead for planting known patterns. \\
Mendeley EU cartel & Procurement & About 15,600 real contracts across seven European countries, drawn from proven cartel cases so that roughly 42\% are cartel-linked. The main procurement test bed. \\
Garc\'ia Rodr\'iguez & Procurement & About 64,000 real bids from six markets (Japan, Italy, Brazil, the USA and two Swiss markets). Used to test whether learning carries from one market to another. \\
...
\item \textbf{GATv2} (graph attention), which learns to pay more attention to
      the neighbours that matter most; this is the model the paper reports as
      its headline deep-learning result on the banking data;
...
flag anything they cannot reconstruct well. Together with a simple structural
baseline, they need no labelled examples, which is why they are the only tools
that can run on the unlabelled Georgian data.
