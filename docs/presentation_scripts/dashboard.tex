...
\slide{Slide 1 --- Title --- One System, Two Crimes}{Medium (6/10)}{$\approx$ 50 seconds}

\begin{saythis}
Good morning everyone. We're Group \blank, and our project is called
CollusionGraph.

We were handed what looks like two completely separate briefs. Find the
money-laundering rings hiding inside a payments network. And find the companies
secretly rigging bids on public contracts. Different industry, different
regulator, different data.

Our claim \bt and this is the thing we'll defend for the next twenty minutes
\bt is that underneath, those are the same problem. Both crimes need more than
one person to work. And the moment a crime needs more than one person, it
leaves a shape behind.

So we built one system that screens both. I'm \blank, with me are \blank\ and
\blank.

We're going to start at the finished end \bt with the screen an investigator
would actually sit in front of \bt and then work backwards into how it's built.
\end{saythis}

\begin{meaning}
This slide is the handshake. You are telling the room four things: who you are,
what you were given, what your claim is, and what order the talk will run in.

The claim sentence is the one that matters. \textbf{``Coordination leaves a
shape''} is your thesis, and every slide after this is evidence for it. Say it
slowly.

Two words to be clear about from the start:
\begin{itemize}
  \item \textbf{Screening} = narrowing a huge list down to the few worth
        looking at. It is not deciding anyone is guilty.
  \item \textbf{The two domains} = money laundering on one side, bid rigging on
        the other. We call them ``money'' and ``contracts'' throughout.
\end{itemize}

Announcing the running order sounds like a small thing. It isn't --- examiners
relax when they know where a talk is going, and a relaxed examiner asks better
questions.
\end{meaning}
...
slide. A working console running on real public data cannot be faked, and
everything in the method section is visible in it.

\q{Is this a real product or a prototype?} It is a research prototype that runs
end to end. Real public datasets, real trained models, real results files. What
it is not is deployed at a bank, and we're careful about that difference.
\end{asked}
...
\begin{saythis}
Let me set up the problem properly, because everything else follows from it.

Money laundering works by moving dirty money through a chain of accounts until
nobody can trace where it started. One account can't do that. You need a chain.

Bid rigging works by companies agreeing in advance who wins which contract, so
the price stays high. One company can't do that either. You need at least two.

So in both cases the crime \emph{is} the coordination. And coordination is
visible \bt not in any one record, but in the pattern between records.

That's why we don't treat this as a spreadsheet problem, where you look at each
customer one at a time. We treat it as a network problem, where what matters is
who deals with whom.

And that single decision is what let us build one system instead of two.
\end{saythis}
