...
**Context-fusion input encoder (added 2026-07-17, user-directed; ablatable):** the GNN family's input layer may fuse the three per-node feature families — raw dataset features, the shared structural template (§4.2 rule 2), and domain screens — through per-family linear encoders combined by a **learned sigmoid gate** (context gating), instead of plain concatenation. Motivation: (a) the post-audit scoreboard shows the GNNs trailing XGB-Graph on Elliptic++ (AUC-PR 0.532 vs 0.810) — an input-representation weakness, which is exactly what the 2025 context-aware GAD literature (context encoding + adaptive aggregation; multi-level feature fusion) targets; (b) it is a strictly additive config option (`fusion: gated|concat`) on the existing §4.4 models — the calibrated score-level ensemble is untouched. Adoption rule: the gated variant ships only if it beats concat on validation AUC-PR under the identical protocol; either outcome is reported as ablation arm B-CF. This is deliberately *feature-level context fusion*, not multimodal/sensor fusion — no new data modalities are invented for it.
...
- **LLM config:** provider/model via env per machine; a cheaper model may be configured for validator agents (`COPILOT_VALIDATOR_MODEL`) to cut cost/latency; each developer uses their own key (never committed). **Preferred provider (2026-07-17): NVIDIA NIM** — the team has NVIDIA Developer Program access, whose `build.nvidia.com` API keys (`nvapi-…`) carry free inference credits (1,000, extensible to 5,000; 40 RPM) against an **OpenAI-compatible endpoint** (`https://integrate.api.nvidia.com/v1`), so the ported OpenAI client needs only a `base_url` + key swap; OpenAI remains the drop-in fallback. This removes the Copilot's API-cost exposure (R16) for the capstone.
...
| R16 | **Copilot LLM cost & latency** — the as-built pipeline takes 20s–10min per query and every agent call is billed | Certain / Medium | Fast-path config (validation stack gated to complex questions — the conditional edges already exist); cheaper model for validator agents; goldens run is a manual CI job, not per-commit; per-developer API keys with spend limits. **2026-07-17: cost leg largely retired — NVIDIA NIM free developer credits (OpenAI-compatible) become the default Copilot provider (§4.6).** |
...
| A13 | (not in problem statement) | **Context-fusion input encoder** for the GNN family (§4.4): gated fusion of raw/structural/screen feature families, config-switchable vs. concat, shipped only if it wins the ablation | User-directed 2026-07-17; motivated by the post-audit GNN-vs-XGB gap and the 2025 context-aware GAD literature; strictly additive and honestly ablated (arm B-CF) |
| A14 | (not in problem statement) | **Deployment & scalability plan** (`docs/deployment.md`): 3-container decomposition (frontend/api/copilot), batch-ML-never-serves rule, AWS free-tier mapping (S3+CloudFront, small EC2 or Lambda-container, credits-funded), Docker adopted at §7 step 22 | User-directed 2026-07-17; production path required by stakeholders; §3.2 batch-offline decision makes it near-zero-cost |
